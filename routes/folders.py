...
def _run_index_job(app, job_id, folder_path_str):
    ...
    with app.app_context():
        ...
        logger.info(
            f"Job {job_id}: completed — {job['indexed']} indexed, "
            f"{job['failed']} failed, {job['skipped']} skipped"
        )


def translate_windows_path(path):
    """Translate Windows paths to container paths on Windows Docker Desktop."""
    import re

    logger.info(f"translate_windows_path received: {path!r}")

    # Match Windows drive letter paths (C:\, D:\, etc.)
    match = re.match(r"^([A-Za-z]):\\(.*)$", path)
    logger.info(f"Regex match result: {match}")

    if match:
        drive = match.group(1).lower()
        rest = match.group(2).replace("\\", "/")
        result = f"/{drive}/{rest}".rstrip("/")
        logger.info(f"Translated {path!r} -> {result!r}")
        return result
    logger.info(f"No match, returning original: {path!r}")
    return path
...
def index_folder():
    ...
    # Translate Windows paths to container paths (e.g., C:\Users -> /c/Users)
    folder_path = translate_windows_path(folder_path)
    folder_path = str(Path(folder_path).resolve())

    logger.info(f"Received path: {data.get('path')!r}")
    logger.info(f"Translated path: {folder_path!r}")
    logger.info(f"Path exists: {os.path.exists(folder_path)}")
    logger.info(f"Path is dir: {os.path.isdir(folder_path)}")
...
def sync_folders():
    ...
    try:
            ...
            for root, dirs, files in os.walk(folder_path):
                for file in files:
                    ...
                    if is_new or is_modified:
                        try:
                            doc_id = str(uuid.uuid5(uuid.NAMESPACE_URL, str_file_path))[
                                :8
                            ]
        ...
        logger.info(
            f"Sync complete: {sync_results['added']} added, "
            f"{sync_results['updated']} updated, {sync_results['skipped']} skipped"
        )

        return jsonify({"success": True, "results": sync_results})
...
def remove_indexed_folder():
        ...
        try:
            ...
            while True:
                results = index.search(
                    "", {"filter": f'folder_root = "{safe_folder_path}"', "limit": 1000}
                )
    ...
    return jsonify(
        {"success": True, "folder": folder_path, "documents_removed": removed_count}
    )
