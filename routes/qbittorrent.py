...
def qbt_save_config():
    ...
    save_searchbox_config(
        current_app._get_current_object(), current_app.Settings, updates
    )
        ...
        for file_path in file_paths:
            ...
            try:
                ...
                if file_stat.st_size > MAX_INDEX_FILE_SIZE:
                    ...
                    logger.warning(
                        f"Skipping oversized file ({size_mb:.0f} MB): {file_path}"
                    )
                    results["files_skipped"] += 1
                    results["errors"].append(
                        f"File too large ({size_mb:.0f} MB, max {MAX_INDEX_FILE_SIZE // (1024 * 1024)} MB): {os.path.basename(file_path)}"
                    )
                    continue

                doc_id = str(uuid.uuid5(uuid.NAMESPACE_URL, file_path))[:8]
                logger.info(
                    f"  Extracting text: {os.path.basename(file_path)} ({file_stat.st_size / 1024:.0f} KB)"
                )
                ...
                results["files_failed"] += 1
                results["errors"].append(
                    f"Error indexing {os.path.basename(file_path)}: {e}"
                )
    ...
    try:
        ...
        while True:
            results = index.search(
                "", {"filter": f'torrent_hash = "{safe_hash}"', "limit": 1000}
            )
