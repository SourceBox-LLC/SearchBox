...
from services.zim_service import (
    index_zim,
    index_zip,
    serve_zim_article,
    serve_zim_image,
    ...
    with app.app_context():
        ...
        try:
            if archive_type == "zim":
                ...
                results = index_zim(
                    archive_path, meili_url, meili_key, INDEX_NAME, progress=job
                )
            ...
            app.IndexedArchive.add(
                archive_path, archive_name, archive_type, indexed_count
            )
...
def remove_archive():
    ...
    try:
        ...
        while True:
            results = index.search(
                "", {"filter": f'folder_root = "{safe_path}"', "limit": 1000}
            )
    ...
    return jsonify(
        {"success": True, "archive": archive_path, "documents_removed": removed_count}
    )
...
def sync_archives():
    ...
    for archive in archives:
        ...
        try:
            ...
            IndexedArchive.add(
                path, archive.archive_name, archive.archive_type, indexed_count
            )
            results["synced"] += 1
