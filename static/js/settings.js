...
// ── Subscription & Team Management (SaaS only) ───────────────────────────────
...
async function loadSubscription() {
  ...
  try {
    ...
    if (planBadge) {
      planBadge.textContent = org.plan.charAt(0).toUpperCase() + org.plan.slice(1);
      if (org.plan === 'enterprise') {
        planBadge.classList.add('enterprise');
      }
    }
    
    // Load usage info from billing plans endpoint
    const plansResp = await fetch('/api/billing/plans');
    const plansData = await plansResp.json();
    
    if (plansData.plans && plansData.plans[org.plan]) {
      const plan = plansData.plans[org.plan];
      if (plan.price !== null) {
        planPrice.textContent = `$${plan.price}/month`;
      } else {
        planPrice.textContent = 'Custom';
      }
    }
    
    // Show billing portal button for paid plans
    const billingPortalRow = document.getElementById('billing-portal-row');
    if (billingPortalRow && ['pro', 'enterprise'].includes(org.plan)) {
      billingPortalRow.style.display = 'flex';
    }
    
    // Show enterprise upgrade for pro users
    const enterpriseUpgradeRow = document.getElementById('enterprise-upgrade-row');
    if (enterpriseUpgradeRow && org.plan === 'pro') {
      enterpriseUpgradeRow.style.display = 'flex';
    }
...
async function loadTeamMembers() {
  ...
  if (!teamMembersEl) return;
  
  try {
    const resp = await fetch('/api/organization/members');
    if (!resp.ok) {
      teamMembersEl.innerHTML = '<p class="loading-text">Unable to load team members</p>';
      return;
    }
    ...
    if (!data.members || data.members.length === 0) {
      teamMembersEl.innerHTML = '<p class="loading-text">No team members yet. Invite people to join your team.</p>';
      return;
    }
    
    teamMembersEl.innerHTML = data.members.map(member => `
      <div class="team-member">
        <div class="member-avatar">${member.name ? member.name.charAt(0).toUpperCase() : member.email.charAt(0).toUpperCase()}</div>
    ...
    const usageMembers = document.getElementById('usage-members');
    if (usageMembers && data.members && data.plan_limits) {
      const memberCount = data.members.length;
      const maxUsers = data.plan_limits.max_users || '∞';
      usageMembers.textContent = `${memberCount} / ${maxUsers === null ? '∞' : maxUsers}`;
    }
...
if (generateInviteBtn) {
  generateInviteBtn.addEventListener('click', async () => {
    ...
    try {
      const resp = await authFetch('/api/organization/invites', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          role: role,
      ...
      if (!resp.ok) {
        inviteError.textContent = data.error || 'Failed to generate invite link';
        generateInviteBtn.disabled = false;
        generateInviteBtn.textContent = 'Generate Link';
        return;
      }
      
      inviteLinkInput.value = data.url;
      generateInviteBtn.textContent = 'Generated!';
      ...
      inviteError.textContent = 'Failed to generate invite link';
      generateInviteBtn.disabled = false;
      generateInviteBtn.textContent = 'Generate Link';
    }
  });
...
if (billingPortalBtn) {
  billingPortalBtn.addEventListener('click', async () => {
    showToast('Billing portal coming soon', 'info');
  });
}
