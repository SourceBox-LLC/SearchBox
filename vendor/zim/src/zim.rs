...
fn parse_article_list(master_view: &Mmap, ptr_pos: u64, count: u32) -> Result<Vec<u32>> {
    // PATCH (SearchBox): the title-ordered article list is optional — callers that
    // iterate by URL never touch it. Modern ZIMs (libzim 6.1+) leave a
    // sentinel/garbage `titlePtrPos`, which made the original `start + count*4`
    // overflow (panic in debug, wrap in release) and then fail to open the file.
    // Compute the range with checked arithmetic and, on overflow or out-of-bounds,
    // return an empty list instead of erroring so the archive still opens.
    let start = usize::try_from(ptr_pos)?;
    let end = match usize::try_from(count)
        .ok()
    ...
    let Some(list_view) = master_view.get(start..end) else {
        return Ok(Vec::new());
    };
